...
class CLASSIFIER:
    ...
    def fit_zsl(self):
        ...
        for epoch in range(self.nepoch):
            ...
            #scipy.io.savemat('test_unseen_features_CZSL_1.mat', {'test_unseen_features':op, 'test_unseen_labels':pl})
            #print('acc %.4f' % (acc))
            if acc > best_acc:
                best_acc = acc
                best_model = copy.deepcopy(self.model.state_dict())
                #scipy.io.savemat('test_unseen_features_CZSL_2.mat', {'test_unseen_features':op, 'test_unseen_labels':pl})
    ...
    def val(self, test_X, test_label, target_classes): 
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            pl = pl.detach().cpu().numpy()
            pl = np.array(pl)
            #scipy.io.savemat('emo_test_unseen_features_CZSL.mat', {'test_unseen_features':op, 'test_unseen_labels':pl}) 
            start = end
        op = test_X
        op = op.cuda()
        op = self.model(op)
